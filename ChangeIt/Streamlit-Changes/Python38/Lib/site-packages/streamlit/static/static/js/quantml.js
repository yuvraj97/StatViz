...
setTimeout(()=>{
    ...
    // Adding meta tags
    _description = document.querySelector('meta[name="description"]')
    _keywords = document.querySelector('meta[name="keywords"]')
...
const observer = new MutationObserver(function(mutations){
    setTimeout(function(){
        document.querySelectorAll("#set-quantml-cookie").forEach(function(element, index){
            document.cookie = `${element.getAttribute("name")}=${element.getAttribute("value")}; path=/;SameSite=Lax`
            // console.log(`Set Cookie: ${element.getAttribute("name")}=${element.getAttribute("value")}; path=/;SameSite=Lax`)
            element.remove()
        })
    }, 0)
    setMetaTags()
