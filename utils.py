...
def getChapterIndexByURL(url):
    ...
    idx = chapters_acronyms_lowercase.index(chapter_acronym) if chapter_acronym != None else 0
...
def set_get_URL(ch=None, dist=None, url=None):
    ...
    url = st.experimental_get_query_params() if url == None else url
    _ch, _dist = "", ""
    add_dist = False
    if (dist != None or "dist" in url): add_dist = True
    if ("ch" in url): _ch = url["ch"][0]
    if ("dist" in url): _dist = url["dist"][0]
    if (ch != None): _ch = ch
    if (dist != None): _dist = dist
