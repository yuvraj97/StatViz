...
def stLoginTitle(sidebar):
    if(sidebar):
        login_title = st.sidebar.empty()
    else:
        login_title = st.empty()
    login_title.title("Login")
    return login_title

def stLoginEmail(sidebar):
    if(sidebar):
        email_txt = st.sidebar.empty()
    else:
        email_txt = st.empty()
    email = email_txt.text_input('Enter your E-mail', value="name@example.com")
    return email_txt, email

def stLoginInvalidEmail(msg, sidebar):
    if(sidebar):
        email_err = st.sidebar.empty()
    else:
        email_err = st.empty()
    email = email_err.error(msg)
    return email

def stLoginIncorrectEmail(msg, sidebar):
    if(sidebar):
        incorrectEmail_warn = st.sidebar.empty()
    else:
        incorrectEmail_warn = st.empty()
    incorrectEmail_warn.warning(msg)
    return incorrectEmail_warn

def stLoginPasswordTitle(msg, sidebar):
    if(sidebar):
        passwd_title = st.sidebar.empty()
    else:
        passwd_title = st.empty()
    passwd_title.title(msg)
    return passwd_title

def stLoginPassword(sidebar):
    if(sidebar):
        passwd_txt = st.sidebar.empty()
    else:
        passwd_txt = st.empty()
    passwd = passwd_txt.text_input("Password", type="password")
    return passwd_txt, passwd

def stLoginIncorrectPassword(sidebar):
    if(sidebar):
        incorrectPassword_warn = st.sidebar.empty()
    else:
        incorrectPassword_warn = st.empty()
    incorrectPassword_warn.warning("Incorrect Password")
    return incorrectPassword_warn

def stLoginResetPassword(sidebar):
    if(sidebar):
        resetPassword_check = st.sidebar.empty()
    else:
        resetPassword_check = st.empty()
    status = resetPassword_check.checkbox("Reset Password")
    return resetPassword_check, status
