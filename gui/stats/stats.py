...
def stDisplay(n_population, n_sample, true_p, n_simulations, GlobalElements):
    ...
    **Probability** tell us what our data look like, so we know that $p = """ + str(true_p) + """$
    ...
    ## Observation   
    We have a population of $"""+ str(n_population) +"""$ Balls, now we draw a sample of $"""+ str(n_sample) +"""$ balls out of these $"""+ str(n_population) +"""$ balls""")
    ...
    **Statistics** gives us an estimate of true "proportion" (that we set to $""" + str(true_p) + """$).    
    ...
    in this case we just see how many balls are $\\color{red}{\\text{red}}$ balls do we have and divide it by total number of balls(=$""" + str(n_sample) + """$).    
    In our sample we have $""" + str(redballs[0]) + """ \\color{red}{\\text{red balls}}$ so for this simulation
    our estimate is $\\hat{p} = """ + str(redballs[0]) + """/""" + str(n_sample) + """=""" + str(redballs[0]/n_sample) + """$
